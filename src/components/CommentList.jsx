      ...
      {comments.map(comment => (
        <div key={comment.id} className="flex justify-between items-start bg-default-50 rounded-lg px-3 py-2">
          <div>
            ...
            <p className="text-xs text-default-400">{comment.author?.email} · {new Date(comment.created_at).toLocaleDateString()}</p>
          </div>
          {isAdmin && (
            <Button color="danger" variant="light" size="sm" onPress={() => deleteComment(comment.id)}>
              Supprimer
            </Button>
          )}
        </div>
